    ...
    <div class="relative w-full rounded-t-lg lg:rounded-t-none lg:rounded-tl-lg lg:rounded-r-lg lg:rounded-br-none bg-[#fff2f2] dark:bg-[#1D0002] overflow-hidden p-6">
        <div class="transition-all delay-300 translate-y-0 opacity-100 duration-750 starting:opacity-0 starting:translate-y-4">
            ...
            <div id="loading-spinner" class="flex justify-center mb-4">
                <svg class="animate-spin h-6 w-6 text-[#F53003]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            <div class="mb-4">
                <div id="progress-container" class="w-full bg-gray-200 rounded-full h-4 dark:bg-[#3E3E3A]">
                    <div id="progress-bar" class="bg-[#F53003] h-4 rounded-full progress-bar-waiting" style="width: 0%"></div>
                </div>
                <p id="progress-text" class="text-sm text-[#706f6c] dark:text-[#A1A09A] mt-2">
                    Проверка: 0 из {{ $checkSession->total_proxies }} прокси
                </p>
                ...
                <table id="results-table" class="w-full text-sm text-left dark:text-[#EDEDEC]">
                    ...
                    @else
                        @foreach ($proxyChecks as $check)
                            <tr class="{{ $check->error_count > 2 ? 'bg-red-100 dark:bg-red-900' : '' }}">
                                <td class="p-2">{{ $check->ip }}:{{ $check->port }}</td>
                                <td class="p-2">{{ $check->type ?? 'Не определен' }}</td>
                                <td class="p-2">{{ $check->country ?? '-' }} / {{ $check->city ?? '-' }}</td>
                                <td class="p-2">{{ $check->status ? 'Работает' : 'Не работает' }}</td>
                                <td class="p-2">{{ $check->response_time ? $check->response_time . ' мс' : 'Таймаут' }}</td>
                                <td class="p-2">{{ $check->external_ip ?? 'Таймаут' }}</td>
                            </tr>
                        @endforeach
                    @endif
                    </tbody>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function updateProgress() {
                ...
                fetch("{{ route('check.status', ['checkSession' => $checkSession->id]) }}")
                    .then(response => {
                        if (!response.ok) {
                    ...
                    .then(data => {
                        ...
                        progressText.textContent = `Проверка: ${checked} из ${totalProxies} прокси`;
                        durationText.textContent = `Время: ${duration} сек.`;
                        ...
                        fetch("{{ route('history.results', ['checkSession' => $checkSession->id]) }}")
                            .then(response => {
                                if (!response.ok) {
                                    throw new Error('Network response was not ok: ' + response.statusText);
                                }
                            ...
                            .then(results => {
                                ...
                                // Используем results.data, так как данные обернуты в объект с ключом "data"
                                ...
                                if (data.length === 0) {
                                    ...
                                    data.forEach(check => {
                                        ...
                                        row.className = check.error_count > 2 ? 'bg-red-100 dark:bg-red-900' : '';
                                        row.innerHTML = `
                                            <td class="p-2">${check.ip}:${check.port}</td>
                                            <td class="p-2">${check.type || 'Не определен'}</td>
                                            <td class="p-2">${check.country || '-'} / ${check.city || '-'}</td>
